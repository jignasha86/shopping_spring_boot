package com.angel.apigateway.config.swagger;
...
public class ServiceDescriptionUpdater {
	...
	private String getSwaggerURL( ServiceInstance instance, String serviceId){		
		...
		String initialUri = instance.getUri().toString();
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(initialUri);
		String modifiedUri = builder.host("localhost").port("8080").toUriString()+"/";
		return modifiedUri+serviceId.trim()+"/v2/api-docs";
		//return swaggerURL != null ? instance.getUri()+swaggerURL : modifiedUri+serviceId+DEFAULT_SWAGGER_URL;
	}
	...
	private Optional<Object> getSwaggerDefinitionForAPI(String serviceName, String url){
		...
		try{
			...
			ResponseEntity jsonData = template.getForEntity(url, Object.class);
